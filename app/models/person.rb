...
class Person < ActiveRecord::Base
  ...
  def self.import(file)
    ...
    def self.main_search(params)
      joins(("LEFT JOIN `contact_telephones` ON contact_telephones.person_id = people.id")).where("first_name LIKE ? OR fathers_maiden_name LIKE ? OR mothers_maiden_name LIKE ? OR email LIKE ? OR contact_telephones.phone_number LIKE ? OR people.id LIKE?", "%#{params[:q]}%", "%#{params[:q]}%", "%#{params[:q]}%", "%#{params[:q]}%", "%#{params[:q]}%" ,"%#{params[:q]}%")
    end
